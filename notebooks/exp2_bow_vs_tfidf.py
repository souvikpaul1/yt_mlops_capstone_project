...
def train_and_evaluate(df):
    with mlflow.start_run(run_name="All Experiments") as parent_run:
        for algo_name, algorithm in ALGORITHMS.items(): #algo_name is the key and algorithm is the value
            for vec_name, vectorizer in VECTORIZERS.items(): #vec_name is the key and vectorizer is the value
                with mlflow.start_run(run_name=f"{algo_name} with {vec_name}", nested=True) as child_run: 
                    try:
                        ...
                        input_example = X_test[:5] if not scipy.sparse.issparse(X_test) else X_test[:5].toarray() # Convert sparse matrix to dense and slice when necessary
